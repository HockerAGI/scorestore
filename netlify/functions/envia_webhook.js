 ...
 * - Notifica pedidos a Telegram y WhatsApp (Meta Cloud API)
 *
 * ENV:
 *  TELEGRAM_BOT_TOKEN
 *  TELEGRAM_CHAT_ID
...
function formatOrder(o) {
  ...
  if (o.shipping?.address) {
    ...
    lines.push(`${a.line1 || ""} ${a.line2 || ""}`.trim());
    lines.push(`${a.city || ""}, ${a.state || ""} ${a.postal_code || ""}`.trim());
  ...
  if (items.length) {
    lines.push("🛒 Items:");
    for (const it of items) lines.push(`- ${it.qty} x ${it.name} (${it.amount_total ?? ""})`);
  }
