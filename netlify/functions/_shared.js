// netlify/functions/_shared.js
const catalogData = require("../../data/catalog.json");

/* =========================
   HELPERS & RESPONSES
========================= */
function jsonResponse(statusCode, body) {
  return {
    statusCode,
    headers: { 
      "Content-Type": "application/json; charset=utf-8",
      "Access-Control-Allow-Origin": "*", // CORS
      "Access-Control-Allow-Headers": "Content-Type"
    },
    body: JSON.stringify(body),
  };
}

function safeJsonParse(raw, fallback = null) {
  try { return JSON.parse(raw); } catch { return fallback; }
}
function digitsOnly(v) { return (v || "").toString().replace(/\D+/g, ""); }
function toStr(v) { return (v || "").toString().trim(); }

/* =========================
   CATALOG LOGIC
========================= */
async function loadCatalog() { return catalogData; }

function productMapFromCatalog(catalog) {
  const map = {};
  // Soporte para estructura { products: [...] }
  const items = Array.isArray(catalog) ? catalog : (catalog.products || []);
  for (const p of items) map[p.id] = p;
  return map;
}

function validateCartItems(items) {
  if (!Array.isArray(items) || !items.length) return { ok: false, error: "Carrito vacío" };
  // Limpiamos datos
  const clean = items.map(i => ({
    id: toStr(i.id),
    qty: parseInt(i.qty) || 1,
    size: toStr(i.size) || "Unitalla"
  }));
  return { ok: true, items: clean };
}

/* =========================
   ENVIA.COM LOGIC (COTIZADOR)
========================= */
async function getEnviaQuote(postalCode, itemsCount = 1) {
  const apiKey = process.env.ENVIA_API_KEY;
  if (!apiKey) {
    console.warn("Falta ENVIA_API_KEY");
    return null; 
  }

  try {
    // 1 prenda = ~0.8kg, cada extra +0.3kg
    const weight = 0.8 + (Math.max(0, itemsCount - 1) * 0.3);

    const payload = {
      origin: {
        company: "UNICO UNIFORMES",
        country_code: "MX",
        postal_code: "22614", // CP Real de Palermo 6106
        state: "BC",
        city: "Tijuana"
      },
      destination: {
        country_code: "MX",
        postal_code: postalCode
      },
      packages: [{
        content: "Ropa Deportiva",
        amount: 1,
        type: "box",
        dimensions: { length: 30, width: 25, height: 10 },
        weight: weight,
        insurance: 0,
        declared_value: 500
      }],
      shipment: { carrier: "fedex", service: "standard" } // Preferencia
    };

    const res = await fetch("https://api.envia.com/ship/rate/", {
      method: "POST",
      headers: { 
        "Content-Type": "application/json", 
        "Authorization": `Bearer ${apiKey}` 
      },
      body: JSON.stringify(payload)
    });

    if (!res.ok) return null;
    const data = await res.json();
    
    // Buscar la tarifa más económica de las confiables
    let rates = (data.data || []).filter(r => 
      ["fedex", "estafeta", "paquetexpress", "dhl"].includes(r.carrier?.toLowerCase())
    );
    
    if (!rates.length) rates = data.data || []; // Fallback a cualquiera
    if (!rates.length) return null;

    // Ordenar por precio
    rates.sort((a, b) => a.total_price - b.total_price);
    const best = rates[0];

    return {
      mxn: Math.ceil(best.total_price),
      label: `${best.carrier.toUpperCase()} Económico`,
      carrier: best.carrier,
      days: best.delivery_estimate || "3-5"
    };

  } catch (e) {
    console.error("Envia Error:", e);
    return null;
  }
}

module.exports = {
  jsonResponse,
  safeJsonParse,
  digitsOnly,
  loadCatalog,
  productMapFromCatalog,
  validateCartItems,
  getEnviaQuote
};
