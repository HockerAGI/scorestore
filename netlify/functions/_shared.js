...
function productMapFromCatalog(c) { const m={}; c.products.forEach(p=>m[p.id]=p); return m; }
  ...
  try {
    const res = await fetch("https://api.envia.com/ship/rate/", {
      ...
      body: JSON.stringify({
        ...
        packages: [{ content: "Ropa", amount: 1, type: "box", weight: qty*0.5, dimensions: {length:30,width:25,height:15}, declared_value: 400*qty }]
      })
    });
    const data = await res.json();
    if (data?.data?.[0]) {
      const best = data.data.sort((a,b)=>a.total_price - b.total_price)[0];
      return { mxn: Math.ceil(best.total_price), carrier: best.carrier,
