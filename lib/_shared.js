...
// FIX: NO usar "*" como valor de Allow-Origin cuando credentials = true.
// Si el origen no está en la lista, devolvemos el VERCEL_PROD_URL como fallback
// (esto bloquea credenciales de orígenes externos pero no rompe la tienda propia).
...
const getOriginByCountry = (country) => {
  ...
  if (c === "US") {
    return {
      name: "Score Store US", company: "Score Store", email: SUPPORT_EMAIL,
      phone: "8180000000", street: "Otay Mesa Rd", number: "123",
      district: "Otay", city: "San Diego", state: "CA", country: "US", postalCode: "92154",
    };
  }
  return {
    name: "Score Store MX", company: "Único Uniformes", email: SUPPORT_EMAIL,
    phone: SUPPORT_PHONE, street: "Palermo", number: "6106",
...
const getPackageSpecs = (country, items_qty) => {
  ...
  if (c === "US") {
    return {
      type: "box", content: "Merchandise", amount: 1,
      weightUnit: "LB", lengthUnit: "IN",
      weight: qty * 0.8, dimensions: { length: 12, width: 12, height: 8 },
    };
  }
  return {
    type: "box", content: "Ropa", amount: 1,
    weightUnit: "KG", lengthUnit: "CM",
    weight: qty * 0.4, dimensions: { length: 25, width: 20, height: 15 },
  };
};
...
const getEnviaQuote = async ({ zip, country, items_qty }) => {
  ...
  const payload = {
    origin,
    destination: {
      name: "Cliente", email: SUPPORT_EMAIL, phone: "0000000000",
      street: "Stripe", number: "1", district: "Centro",
      city: zipInfo?.city || "Tijuana", state: zipInfo?.state || "BC",
      country: c, postalCode: z,
    },
  ...
  const res  = await fetch(`${ENVIA_API_URL}/ship/rate`, {
    method: "POST", headers: enviaHeaders(), body: JSON.stringify(payload),
  });
  const data = await res.json();
...
// FIX: apiVersion actualizada a la versión estable actual
const initStripe = () => {
 ...
 * FIX: Esta función faltaba en _shared.js pero era importada por _create_checkout.js.
 * Genera una clave determinista a partir del email, carrito y zip del pedido.
 */
const makeCheckoutIdempotencyKey = (req, body = {}) => {
  try {
    ...
    const zip = String(
      body?.shipping_zip || body?.postal_code || body?.zip || ""
    ).trim();
...
const readPublicSiteSettings = async (sb = null, orgId = null) => {
  ...
  const { data }   = await client
    .from("site_settings")
    .select("*")
    // FIX: usar organization_id (columna real en el schema)
...
module.exports = {
  ...
  makeCheckoutIdempotencyKey,   // ← FIX: antes faltaba el export
  SUPPORT_EMAIL,
  SUPPORT_PHONE,
