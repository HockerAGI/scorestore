...
function normalizeCategory(row) {
  ...
  return {
    ...
    title: safeStr(row?.title || row?.name || id.replace(/_/g, " ")).trim(),
    logo: cover,
    cover_image: cover,
    image: cover,
...
function buildSectionsFromProducts(list) {
  ...
  for (const p of Array.isArray(list) ? list : []) {
    ...
    if (!map.has(key)) {
      const cover = normalizeAssetPath("/assets/logo-score.webp");
      map.set(key, {
        id: key,
        uiId: key,
...
function buildCatalogResponse(source = {}) {
  ...
  const categoriesOut = rawCategories.length
    ? rawCategories.map(normalizeCategory).filter(Boolean)
    : buildSectionsFromProducts(productsOut);
...
async function selectByOrgSingle(sb, table, columns, orgId) {
  if (!sb) return null;
...
async function loadCatalogFromJsonOrDb(orgId = "") {
  ...
  if (!sb) return buildCatalogResponse({ products: [], categories: [], sections: [] });
...
function buildPublicPrompt({ store, stats, products, categories, context }) {
  ...
  const publicWhatsApp = safeStr(contact.whatsapp_display || process.env.SUPPORT_WHATSAPP_DISPLAY || "664 236 8701");
