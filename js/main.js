/* =========================================================
   ...
   - Lógica de UI / UX / Carrusel Restaurado.
   - Neuromarketing: Ventas Reales (Conectado a DB Supabase).
   - VULNERABILIDAD RESUELTA: Idempotencia real en Stripe (req_id).
   - HARDENING: Eliminación de inline handlers para CSP Estricto.
   ========================================================= */
...
(() => {
  ...
  // ---------- SISTEMA DE NEUROMARKETING (VENTAS REALES) ----------
  const fetchActivityFeed = async () => {
    try {
      ...
      if (data.ok && data.events && data.events.length > 0) {
        // Seleccionamos un evento al azar de los recientes
        return data.events[Math.floor(Math.random() * data.events.length)];
      }
    } catch (e) { console.warn("Activity feed offline"); }
    return null;
  };
  ...
  const triggerSalesNotification = async () => {
    ...
    if (!event) return; // Si no hay datos reales, no mostramos mentiras
  ...
  const initNeuromarketing = () => {
    setTimeout(() => {
      ...
      setInterval(() => triggerSalesNotification(), Math.floor(Math.random() * 20000) + 40000); // 40-60s
    }, 12000);
  };
  ...
  const normalizeProduct = (p) => {
    ...
    return { 
        sku, id: sku, title, description: String(p?.description || "").trim(), priceCents, 
        images: images.map(safeUrl), img: images[0] ? safeUrl(images[0]) : "", 
        sizes: sizes.map((s) => String(s || "").trim()).filter(Boolean), 
        rawSection, uiSection: normalizeSectionIdToUi(rawSection) || "BAJA1000", 
        collection: inferCollection(p), rank: Number.isFinite(Number(p?.rank)) ? Number(p.rank) : 999 
    };
  };
  ...
  const fetchPromos = async () => {
    try {
      const res = await fetch(`data/promos.json?cv=${encodeURIComponent(APP_VERSION)}`, { headers: { "cache-control": "no-store" } });
      ...
      card.innerHTML = `
        ...
        <div class="card__body">
          ...
          <div class="card__row">
            ...
            <div style="display:flex; gap:5px;">${logoPill} ${colPill}</div>
          </div>
  ...
  const openProduct = (sku) => {
    ...
    if (pmChips) {
      pmChips.innerHTML = `<span class="pill pill--logo"><img src="${safeUrl(getLogoForSection(p.uiSection))}" alt="Logo"></span>`;
    ...
    if (pmCarousel) {
      const imgs = p.images.length ? p.images : (p.img ? [p.img] : []);
      pmCarousel.innerHTML = `<div class="pm__track" id="pmTrack">${imgs.map((src) => `<img src="${safeUrl(src)}" loading="lazy">`).join("")}</div>
  ...
  const validatePromo = () => {
    ...
    if (!code || !promosData?.rules) { activePromo = null; return; }
    
    const p = promosData.rules.find(x => x.code === escapeHtml(code) && x.active);
    if (p && (!p.expires_at || new Date() <= new Date(p.expires_at)) && (cartSubtotalCents(false) / 100) >= (p.min_amount_mxn || 0)) {
      activePromo = p; return;
    }
    activePromo = null;
  };
  ...
  const cartSubtotalCents = (applyDiscount = false) => {
    const sub = cart.reduce((sum, it) => {
      ...
      return sum + (Math.max(0, Number(realProduct ? realProduct.priceCents : it.priceCents)) * Math.max(1, Number(it.qty || 1)));
    }, 0);
  ...
  const renderCart = () => {
    ...
    if (!cart.length) {
      // FIX CSP: DOM creation instead of innerHTML strings with onclick
    ...
    clearBtn.addEventListener("click", () => {
      if(confirm("¿Estás seguro de vaciar tu carrito?")) { cart = []; saveCart(); validatePromo(); renderCart(); }
    });
    ...
    if (discountLineWrap && discountLineEl) {
      if (subNet < subGross && activePromo && activePromo.type !== 'free_shipping') {
        discountLineWrap.hidden = false; discountLineEl.textContent = `-${money(subGross - subNet)}`;
  ...
  const getCheckoutReqId = () => {
    ...
    const now = Date.now();
    // Válido por 10 minutos (600000 ms)
  ...
  const doCheckout = async () => {
    ...
    try {
      const payload = { 
          ...
          promo_code,
          req_id: getCheckoutReqId() // <--- Previene doble cobro
      };
      ...
      sessionStorage.removeItem('score_checkout_req_id'); // Limpiamos al éxito
  ...
  const sendAssistantMsg = async () => {
    ...
    const ctx = {
      ...
      cartTotal: money(cartSubtotalCents() / 100)
    };
    ...
    try {
      ...
      if (actionCartMatch) {
          ...
          setTimeout(() => { closeLayer(assistantModal); openLayer(cartDrawer); refreshShippingUI(); renderCart(); }, 1500); 
      }
  ...
  const openAssistantChat = () => {
    closeLayer(sideMenu); openLayer(assistantModal);
  ...
  const initEvents = () => {
    ...
    // Fix inline handler for Continue Shopping
    ...
    pmAdd?.addEventListener("click", () => {
      ...
      pmAdd.style.backgroundColor = "#28a745"; pmAdd.style.borderColor = "#28a745"; pmAdd.disabled = true; 
      
      setTimeout(() => {
        pmAdd.innerHTML = originalText; pmAdd.style.backgroundColor = ""; pmAdd.style.borderColor = ""; pmAdd.disabled = false;
