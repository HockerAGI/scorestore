/* ======================================================
   SCORE STORE — LOGIC CONTROLLER (V_FINAL)
   ====================================================== */
const API_BASE = (location.hostname.includes('netlify')) ? '/.netlify/functions' : '/api';
const CART_KEY = "score_cart_prod_final_v2";
...
async function init() {
  ...
  try {
    ...
    catalog = data.products || []; // Adaptado a la estructura del nuevo JSON
    
    // Si queremos mostrar un producto destacado en el Hero (ej. Chamarra 2025)
    const featured = catalog.find(p => p.tags.includes('featured')) || catalog[0];
    if(featured && $("featuredBtn")) {
       $("featuredBtn").onclick = () => openCatalog(featured.sectionId, "DESTACADO");
    }
...
window.openCatalog = (secId, title) => {
  ...
  if (!items.length) { 
    $("catContent").innerHTML = "<div style='padding:40px; text-align:center;'>Productos agotados o no disponibles.</div>"; 
    return; 
  }
  ...
  $("catContent").innerHTML = `<div class="catGrid">` + items.map(p => {
    ...
    // Badges según status/tags del JSON
    let statusBadge = "";
    if (p.status === "low_stock") statusBadge = `<div class="status-tag" style="background:#000;">ÚLTIMAS PIEZAS</div>`;
    else if (p.tags.includes("new")) statusBadge = `<div class="status-tag" style="background:var(--score-blue);">NUEVO</div>`;
...
window.add = (id) => {
  ...
  if (exist) exist.qty++;
    ...
    qty: 1, 
    img: p.img,
    sku: p.sku // Importante para inventario
  });
...
function renderCart() {
  ...
  if (!cart.length) {
    wrap.innerHTML = ""; $("cartEmpty").style.display = "block"; updateTotals(); return;
  }
