  ...
  if (!items.length) {
    ...
    grid.innerHTML = items.map(p => `
      <div class="prodCard">
        ...
        <div style="color:var(--red); font-weight:900;">$${p.baseMXN}</div>
        <select id="size_${p.id}" style="margin:5px 0;">
          ${(p.sizes || ['Unitalla']).map(s => `<option>${s}</option>`)}
        </select>
        <button class="btn primary" style="width:100%;"
          onclick="add('${p.id}','${p.name}',${p.baseMXN},'${p.img}')">AGREGAR</button>
      </div>`).join('');
  }
...
function updateCart(resetShip = true) {
  ...
  $('cartBody').innerHTML = cart.map((i, x) => `
    ...
    </div>`).join('') || '<div style="text-align:center; padding:40px 20px; opacity:0.5;">Tu carrito está vacío</div>';
